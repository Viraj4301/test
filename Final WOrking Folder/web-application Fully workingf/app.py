...
@app.route('/downloadsa', methods=['POST'])
def downloadsa():
    rar_file_path = './standalone/Secure-Text-File-Transfer-main.rar'
    return send_file(rar_file_path, as_attachment=True, download_name='Secure-Text-File-Transfer-main.rar')
	...
	for root,dirs,files in os.walk('./media/public-keys/'):
		for file in files:
			...
			if list[0] == username:
				...
				return send_file(filename,as_attachment=True, download_name='publicKey.pem')

@app.route('/file-directory/retrieve/file/<filename>')
def download_file(filename):
	filepath = UPLOAD_FOLDER+filename
	if(os.path.isfile(filepath)):
		return send_file(filepath, as_attachment=True, download_name='fileMessage-thrainSecurity.txt')
	else:
